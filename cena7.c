#include "ORIGEM2.h"

void cena7(Jogo* jogo) {
	SDL_SetRenderDrawColor(jogo->renderer, 173, 134, 79, 255);
	SDL_RenderClear(jogo->renderer);

	SDL_Color preto = { 0, 0, 0, 0 };
	SDL_Surface* Stexto = TTF_RenderText_Solid(jogo->fonte, "Siri: Mais um dia de trabalho!", preto);
	SDL_Texture* texto = SDL_CreateTextureFromSurface(jogo->renderer, Stexto);
	SDL_FreeSurface(Stexto);

	SDL_Rect polirect = { 0, 0, 1768, 992 };
	SDL_RenderCopy(jogo->renderer, jogo->politecnico, NULL, &polirect);

	SDL_Rect rect = { 484, 92, 1000, 1000 };
	SDL_RenderCopy(jogo->renderer, jogo->iris, NULL, &rect);

	SDL_Rect rect3 = { 0, 872, 1768, 120 };
	SDL_RenderFillRect(jogo->renderer, &rect3);

	SDL_Rect rectTexto = { 20, 892, 600, 100 };
	SDL_RenderCopy(jogo->renderer, texto, NULL, &rectTexto);

	SDL_RenderPresent(jogo->renderer);

	SDL_DestroyTexture(texto);
}